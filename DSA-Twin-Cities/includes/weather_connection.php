...
try {
    ...
    $stmt_read_weather = $conn->prepare("SELECT * FROM weather WHERE city_woeid ='".$city->getWoeid()."' AND forecast_id IS NOT NULL;");
    $stmt_read_weather->execute();
    $temp_weather = new Weather();
    foreach ($stmt_read_weather->fetchAll() as $item)
    {
        $temp_weather->setTemperature($item['temp']);
        $temp_weather->setHumidity($item['humidity']);
        $temp_weather->setWindSpeed($item['wind_speed']);
        $temp_weather->setWindDirection($item['wind_direction']);
        $temp_weather->setIcon($item['icon']);
        $temp_weather->setDesc($item['descr']);
        $temp_weather->setWeatherTime($item['weather_time']);
        $temp_weather->setWeatherSaved($item['weather_saved']);
    }

    //Find forecast
    $temp_forecast = new Weather();
    $stmt_read_forecast = $conn->prepare("SELECT * FROM weather WHERE city_woeid ='".$city->getWoeid()."' AND forecast_id IS NULL;");
    $stmt_read_forecast->execute();
    foreach ($stmt_read_forecast->fetchAll() as $forecast)
    {
        $temp_forecast->setTemperature($forecast['temp']);
        $temp_forecast->setHumidity($forecast['humidity']);
        $temp_forecast->setWindSpeed($forecast['wind_speed']);
        $temp_forecast->setWindDirection($forecast['wind_direction']);
        $temp_forecast->setIcon($forecast['icon']);
        $temp_forecast->setDesc($forecast['descr']);
        $temp_forecast->setWeatherTime($forecast['weather_time']);
        $temp_forecast->setWeatherSaved($forecast['weather_saved']);
    }
...
{ //Data is old
    ...
    try {
        ...
        //Update data
        $update_current_weather = "UPDATE weather SET temp=".$current_weather->getTemperature().
            ",humidity=".$current_weather->getHumidity().", wind_speed =".$current_weather->getWindSpeed().
            ", wind_direction=".$current_weather->getWindDirection().", icon ='".$current_weather->getIcon().
            "', weather_time =".$current_weather->getWeatherTime().", weather_saved =".$current_weather->getWeatherSaved().
            ", descr='".$current_weather->getDesc().
            "' WHERE city_woeid=".$city->getWoeid()." AND forecast_id IS NOT NULL";

        $update_forecast_weather = "UPDATE weather SET temp=".$forecast_weather->getTemperature().
            ",humidity=".$forecast_weather->getHumidity().", wind_speed =".$forecast_weather->getWindSpeed().
            ", wind_direction=".$forecast_weather->getWindDirection().", icon ='".$forecast_weather->getIcon().
            "', weather_time =".$forecast_weather->getWeatherTime().", weather_saved =".$forecast_weather->getWeatherSaved().
            ", descr ='".$forecast_weather->getDesc().
            "' WHERE city_woeid=".$city->getWoeid()." AND forecast_id IS NULL";

        $stmt = $conn->prepare($update_current_weather);
        $stmt->execute();
        $stmt = $conn->prepare($update_forecast_weather);
        $stmt->execute();
        $conn = null;
    }
    catch (PDOException $e)
...
function displayCurrentAndForecast($current,$forecast)
{
    ...
    $output .= '<tr>';
        ...
        $output .= '<td>'.($current->getWindDirection() != -10 ? degreeCompassPointConv($current->getWindDirection()).' ('.round($current->getWindDirection()).'°)' : 'N/A').'</td>';
        $output .= '<td>'.degreeCompassPointConv($forecast->getWindDirection()).' ('.round($forecast->getWindDirection()).'°)</td>';
    $output .= '</tr></table>';
